...
| Dashboard Playground returns `404`                            | Two distinct root causes — both already fixed in the generated stack: (1) the router runs a Gemini startup probe and refuses to start with `[FATAL] ...` if the API key is missing/invalid or the model is unreachable; check the router logs right after deploy — Gemini's OpenAI-compat endpoint accepts bare model names (`gemini-3.1-flash-lite`, `gemini-3.5-flash`) — **not** the `google/...` prefix (Vertex AI only). (2) the dashboard frontend attaches its session token as `Authorization: Bearer ...` to `/api/router/v1/chat/completions`; the dashboard backend forwards it to envoy; the router's ExtProc filter then **appends** its own `Authorization: Bearer <GEMINI_KEY>` (envoy default is `APPEND_IF_EXISTS_OR_ADD`); Gemini receives two Authorization values and returns an empty-body 404. The generated `envoy.yaml` ships with a tiny Lua filter that runs before ExtProc and strips the incoming Authorization, so only the router-injected Gemini key reaches Gemini. |
