...
| Dashboard Playground returns `404`                            | The router runs a Gemini startup probe and refuses to start with `[FATAL] ...` if the API key is missing/invalid or the model is unreachable. Check the router container logs right after deploy — the FATAL line tells you exactly which case it is. Gemini's OpenAI-compat endpoint accepts bare model names (`gemini-3.1-flash-lite`, `gemini-3.5-flash`) — **not** the `google/...` prefix (Vertex AI only).             |
